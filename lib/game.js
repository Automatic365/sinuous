...
function Game(canvas, context){
  ...
  function collisionDetectionGameElement (player, collection, createGameElements) {
    collection.forEach(function(element, index, collection) {
      ...
      if (distance < player.radius + element.radius) {
        if (element.color === "green") {
          contextStyle(element.color);
          collection.splice(index, 1);
          createGameElements(1, collection, Powerup);
          oneUp();
        } else {
          contextStyle(element.color);
          enemies.splice(index, 1);
          createGameElements(1, collection, Enemy);
          player.deductLife();
        }
      }
    });
  ...
  requestAnimationFrame(function gameLoop() {
    ...
    collisionDetectionGameElement(player, powerups, createGameElements);
    collisionDetectionGameElement(player, enemies, createGameElements);
